...
class TestFakeMoneyTypeUtils(unittest.TestCase):
    def test_nested_dict_get_aggreaged_tautology(self):
        class aggegated:
            get = lambda j, i: True
        value = fakemoney.types.nested_dict_get(aggegated, None)

    def test_nested_dict_get_aggreaged_dict_valid(self):
        test_value = "test value"
        test_dict = dict(value=test_value)
        value = fakemoney.types.nested_dict_get(test_dict, "value")
        assert value == test_value

    def test_nested_dict_get_aggreaged_dict_invalid(self):
        test_value = "test value"
        test_dict = dict(value=test_value)
        value = fakemoney.types.nested_dict_get(test_dict, "invalid")
        assert value is None

    def test_is_valid_constructor_call(self):
        is_valid_func = fakemoney.types.isValidConstructor({})
    ...
    def test_is_valid_constructor_call_fields_builtin_data_empty(self):
        fields = { "name": str }
        is_valid_func = fakemoney.types.isValidConstructor(fields)
        data = {}
    ...
    def test_is_valid_constructor_call_fields_custom_data_valid(self):
        fields = { "name": str, "age": int, "score": float, "email": fakemoney.types.email}
        is_valid_func = fakemoney.types.isValidConstructor(fields)
